...
배열을 길이 $2^0$의 조각들로 분할하여 각 조각의 최솟값을 저장해 두고, $2^1$의 조각들로 분할하여 저장해 두고, …, $2^{\lfloor \log N \rfloor}$의 조각들로 분할하여 저장해 둡시다.
모든 구간 $[l, r)$은 다음과 같이 최대 $2 \log_2 N$ 개의 구간으로 분할할 수 있습니다.

$$
a_0 \lt a_1 \lt \cdots \lt a_m \gt a_{m+1} \gt \cdots \gt a_k
$$
...
모든 $i$와 $1$ 이상 $log_2 N$ 이하의 모든 $a$에 대해 구간 $[i, i + a)$에서의 최솟값을 저장해 둔다면 최소 공통 조상을 $\mathcal{O}(1)$ 시간에 구할 수 있습니다.
